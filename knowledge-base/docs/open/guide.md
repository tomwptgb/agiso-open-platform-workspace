# 开放平台 (Open) - 接入指南

### 接入指南

#### 1、接入流程

【开发者操作】登录后台申请AppId。申请时要选择应用类型为【托管服务】

https://open.agiso.com/#/my/application/app-list

【开发者操作】申请到AppId后，开发者可以登录后台管理AppId，这里可以查看和更换AppSecret、更改推送url、更改授权回调url等。

【商家操作】授权第三方管理应用：

1、先申请一个开放平台账号。然后登录开放平台。

https://open.agiso.com

2、访问以下页面进行授权

【开发者操作】开发者得到各个商家授权给的Token，并使用Token调用接口。调用接口时，需要使用托管服务AppSecret进行签名，具体签名方法参见下文。

注意：开发者与商家，也可以是同一个人。

#### 2、获取AccessToken详解

暂不支持手动模式

2、引导用户登录授权

引导用户通过浏览器访问以上授权url

3、获取code

用户点“确认”按钮后，Agiso开放平台会将用户重定向到托管服务填写的回调地址上，并附带上 授权码code、自定义参数state 作为参数，应用可以获取并使用该code去换取AccessToken

4、换取AccessToken

换取AccessToken返回值示例

换取AccessToken返回参数说明

#### 3、调用接口详解

注意：接口调用配额，20次/秒。

#### 4、签名算法

#### 5、Header设置示例代码

#### 6、签名算法示例代码

#### 7、完整调用API示例代码

以下代码以调用LogisticsDummySend(更新发货状态)为例

开放平台托管服务，专为不熟悉开放平台操作流程的商家设计，通过托管模式简化应用创建、信息修改、密钥重置等操作，降低商家使用门槛。

托管服务应用获得商家授权后，可通过本文档接口代商家完成应用全生命周期管理；商家使用托管电商业务应用时，其操作流程与自行创建的电商业务应用完全一致。

托管服务创建

：开发者需先创建托管服务应用。

托管服务授权

：商家将自身开放平台账号授权给该托管服务。

托管应用创建管理

：托管服务为已授权的商家创建对应的托管电商业务应用，并可通过开放平台接口完成应用信息修改、secret
              重置等操作。

商家后台授权

：商家获取托管电商业务应用的 AppId，在自身自动发货后台完成授权配置，即可投入使用。

注意事项：

- 单个开放平台账号，可授权给

多个托管服务

（支持授权给自身申请的托管服务，便于开发调试）。

- 单个托管服务，

仅能为每个授权账号创建 1 个托管电商业务应用

- 托管电商业务应用本质为标准应用，与普通电商业务应用的区别仅在于：支持通过开放平台接口修改数据，其余使用功能完全一致。

- 权限继承规则：托管电商业务应用无需单独向客服申请权限，

自动继承对应托管服务的所有接口调用与消息推送权限

，仅需为托管服务完成客服侧权限配置。

- 资质简化规则：

托管电商业务应用对应的开放平台账号无需进行资质认证。

【对所有API请求参数（包括公共参数和业务参数，但除去sign参数和byte[]类型的参数），根据参数名称的ASCII码表的顺序排序。如：foo=1, bar=2, foo_bar=3,
        foobar=4排序后的顺序是bar=2, foo=1, foo_bar=3, foobar=4。

将排序好的参数名和参数值拼装在一起，根据上面的示例得到的结果为：bar2foo1foo_bar3foobar4。

把拼装好的字符串采用utf-8编码，在拼装的字符串前后加上app的secret后，使用MD5算法进行摘要，如：md5({$托管服务的AppSecret}bar2foo1foo_bar3foobar4{$托管服务的AppSecret})

```
{
    "IsSuccess": true,
    "Error_Code": 0,
    "Error_Msg": "",
    "Data": {
        "FromPlatform": "Open",
        "UserId": "sub-d54fca6facc341508c09a420dffb",
        "UserNick": "manage-subapp",
        "ExpiresIn": 2341404360,
        "DeadLine": "2099-12-31 00:00:00",
        "Token": "Openzxgg6shp5nm2dds36dtb4vae8s9dgat2"
    },
    "SuccessCount": 0,
    "FailedCount": 0
}
```

| 参数名 | 必选 | 类型 | 说明 |
|--------|------|------|------|
| appId | 是 | long | 托管服务的AppId |
| state | 否 | string | 开发者自定义参数，授权回调会把该参数回传回去 |
